...
void inline getCIGAR(SeqPair *p, int16_t *cigarBuf_, int32_t cigarBufLength, int32_t tid)
{
    ...
    // The length of the string can be truncated to int16_t because it will never be larger than the at most 16 bit long input.
    p->cigarCount = (int16_t)strnlen(p->cigar, curSize);
}
