...
public class IntelPDHMMUnitTest {
	...
	public void pdhmmPerformanceTest() {
		...
		try {
			...
			// todo - do we need to close and recreate fis ?
			fis.close();
			...
			while ((line = br.readLine()) != null) {
				...
				appendArray(alleleBasesFull, alleleBases, currentTestcase * HAPLOTYPE_MAX_LENGTH);
				appendArray(allelePDBasesFull, allelePDBases, currentTestcase * HAPLOTYPE_MAX_LENGTH);
				appendArray(readBasesFull, readBases, currentTestcase * READ_MAX_LENGTH);
				appendArray(readQualsFull, readQuals, currentTestcase * READ_MAX_LENGTH);
				appendArray(readInsQualsFull, readInsQuals, currentTestcase * READ_MAX_LENGTH);
				appendArray(readDelQualsFull, readDelQuals, currentTestcase * READ_MAX_LENGTH);
				appendArray(overallGCPFull, overallGCP, currentTestcase * READ_MAX_LENGTH);
	...
	private void appendArray(byte[] mainArray, byte[] appendArray, int index) {
		for (int i = 0; i < appendArray.length; i++) {
			mainArray[index + i] = appendArray[i];
		}
	}
}